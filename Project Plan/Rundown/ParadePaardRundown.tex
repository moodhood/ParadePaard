...
ParadePaard is a workforce application for employee planning, worked hours, travel claims, leave requests, payslips, account details, and company administration. From the frontend point of view, the application connects two groups of users. Employees use it to manage their own work life: they log in, finish onboarding, view their upcoming shifts, accept or decline planning requests, check worked hours, submit travel claims, request leave, download payslips, report payslip problems, and view their own account information. Admins and authorized managers use the same app to run the operation: they onboard employees, manage planning events and shifts, assign people to work, review leave and travel claims, inspect timesheets, manage roles and company settings, and review or correct payslips.

The app is held together by a top navbar, a primary navigation rail, and access rules that decide which pages a person can use. The result is a role-aware experience: a regular employee is guided toward personal pages, while an admin sees management pages unless they explicitly switch into personal view.
...
The dashboard entry point decides whether to show the admin dashboard or the employee dashboard. A non-admin user sees the User Dashboard. An admin normally sees the Admin Dashboard. If an admin switches into personal view, the app deliberately shows the User Dashboard instead, so the admin can inspect and use their own employee-facing account experience.

This is one of the clearest examples of the app's role-based frontend behavior. The same dashboard entry can produce a different page depending on the user's role and whether the admin has switched into personal view.
...
The Admin Dashboard is the operational overview for the company. Instead of focusing on one employee, it summarizes team-wide work and management tasks. It is laid out as a grid of cards, each one pointing to a different admin responsibility.

The General Info card shows the current week, team hours this week, team hours this month, team hours this year, total users, active versus pending setup users, pending leave requests, and payslips pending review. A button in the card opens the planning overview, which is the next likely admin action when looking at staffing.

The Hours Worked card lets the admin inspect timesheets either as a per-user summary or as a timesheet history list. The admin can choose a timeframe type: week, month, year, or all time. For weekly and monthly views, extra selectors let the admin choose the year, week number, or month. In the per-user view, employee names are links to their admin user detail pages. In the timesheet view, the card shows date, employee name, function, and hours. A footer shows the total hours for the current filter.

The Pending Requests card is primarily used for pending leave requests. It shows who submitted the request, the request type, and the request date. Clicking a leave request opens the admin leave decision modal. In that modal, the admin can review the requested dates, requested hours, available hours, remaining hours if approved, the employee's note, and an optional decision note. The admin can approve or reject the request from the modal.

The Payslip Errors card focuses on payslips that are disputed, need attention, or include an error description. It shows the employee name, a short issue label, and a date. The Review payslip issues button opens the all-payslips list filtered to not-accepted payslips. Clicking an issue row opens the admin payslip editor.

The Contract End card shows contracts ending in the next month. It gives the employee name, end date, and days left. Employee names link to their admin detail pages. The Payslip Review card shows payslips pending review, with employee name, payout date, and status. A button opens the dedicated payslip review page. Together, these cards make the dashboard a triage page: the admin can see where action is needed and jump directly into the relevant workflow.

\subsection{Admin Users Page}

The Admin Users page is the employee directory. It shows all users in a paginated table, with each row presenting the user's avatar or initials, name, user ID, email, position, roles, date added, and status.

The toolbar above the table gives admins the controls they need to find a person quickly. They can search by name or email, sort by name, status, position, or date added, change the sort direction, filter by tenure, refresh the data, and move through pages. Clicking a row opens the detail page for that employee. The page is primarily for admins because it exposes company-wide employee information.

\subsection{Admin User Detail Page}

The Admin User Detail page is the complete admin view of one employee. The top of the page works like a profile header. It includes a back button, the employee's name, a status badge, email, position, location, role count, and summary metrics such as recorded hours, upcoming shifts, and last timesheet.

The page is organized into Profile, Timesheets, and Planning tabs. The Profile tab reads like an employee dossier. It shows personal information, work details, active contract details, employee tax profile, roles and access, address, and bank details. The active contract section includes a contract PDF download. The employee tax profile section includes fields for BSN, loonheffingskorting, pension participant, special Zvw contribution, and payroll notes. If the admin has the right permissions, they can save the tax profile, add roles, or remove roles. Role chips use colors, and the role add menu is permission-aware.

The Timesheets tab starts with summary cards for total hours, entry count, average entry, and travel expenses. It then includes a form for logging worked hours, with date, function, hours worked, and travel expenses. Below that, the timesheet history can be filtered by week, month, year, or all time. The history rows show date, function, week, hours, travel, recorded status, and approval state.

The Planning tab shows the employee's relationship to scheduled work. It summarizes active, upcoming, accepted, and past shifts. The lists use planning status labels such as Scheduled, Accepted, and Declined, and related planning rows can link back into the event and shift management screens. This page is therefore where an admin connects a person's account, contract, payroll setup, worked hours, and planning record.

\subsection{Admin Employee Onboarding Page}

The Admin Employee Onboarding page starts the new-employee flow. It presents one main employee details form. The admin enters identity and contact data such as preferred name, first names, prefix, surname, gender, date of birth, email, and mobile number. The form also asks whether the person has worked for the company before.

The same screen captures initial employment and contract information. The admin selects a position or function, enters contract start and end dates, chooses contract type, supplies or accepts a gross hourly wage, and chooses whether the employee has travel allowance. If the selected function already provides a wage, the wage field is disabled and follows that function wage. Date inputs are formatted as the admin types.

When the admin submits the form, the result area shows the created username, temporary password, contract information if returned, and the onboarding email status. From a user-interface point of view, this page turns a candidate into an invited employee who can then log in, reset their password if required, and complete onboarding.

\subsection{Account Layout}

The Account page is a layout wrapper for personal account pages and company settings. It shows the shared navbar, a back button, a page title, side navigation, and the active account content. The personal account side navigation contains Personal info, Bank details, and Employment details. When the user is on company settings, the side navigation changes to Company details, Roles and permissions, Workflow settings, and Tax settings.

The account layout loads the current user and passes common account data to the nested account pages. It also owns the profile-picture upload and removal behavior used by the Personal info page. If an admin is in personal view, company settings are blocked and the user is redirected back to the personal account area. This prevents the personal employee experience from mixing with admin company configuration.
...
Company Settings is a multi-tab admin area inside the account layout. It is the place where authorized users manage company identity, access roles, workflow settings, and payroll deduction templates.
...
\subsection{Admin Planning Overview Page}
...
\subsection{Admin Planning Clients Page}
...
\subsection{Admin Planning Event Detail Page}
...
The Work History page shows logged timesheets. Employees normally see their own timesheets. Users with \code{CAN_VIEW_ALL_TIMESHEETS} can see all employees' timesheets when they are not in personal view. If the user can manage timesheets and is not in personal view, the page header includes a button to open Travel Claims.

The main content is a Timesheets card with a filter panel, table, total bar, and pagination. The filter panel is rich enough to support both personal and admin views. The user can search by function, event, shift, date, name, or employee ID depending on scope. They can filter by employee in all-timesheets view, by function, by date range, by week year and week number, by minimum and maximum hours, and by minimum and maximum travel amount. The Reset filters button clears the filter state.
...
The Travel expenses card shows the travel amount stored on the timesheet, submission status, kilometers, rate, submitted-at time, reviewed-at time, claim amount, and review note. If the claim includes proof, the page shows a View proof button. Admin-capable users outside personal view use admin proof access, while employees use their own assignment proof access.
...
The Payslips page is the main list for payroll documents. Employees see their own payslips. Users who can view all payslips can switch between My payslips and All payslips unless they are in personal view. The list shows date, user when in all scope, week, function, hours, net pay, status, and an action column.

The page includes a detailed filter panel. Users can search, filter by status, choose date range, choose week year and week number, and filter by minimum or maximum hours and net pay. The special status filter \code{NOT_ACCEPTED} is a frontend convenience for focusing on payslips that are not released or approved. It is used from admin review flows to quickly find problematic payslips.

The user can download a payslip PDF from the list, open a detail page by selecting a row, reset filters, and use pagination. For employees, this is a payroll archive. For admins, it is also a review and troubleshooting entry point.
...
\subsection{Admin Payslip Detail Page}

The Admin Payslip Detail page is the payroll editor. It uses the same core payslip information as the employee detail page, but turns much of it into editable fields and adds review tools. The top actions let the admin download the PDF, open the employee's user detail page, and save changes.

The Overview card shows payslip ID, editable issue date, week, available-to-user time, and generated-at time. The Error handling card lets the admin set the payslip status and edit the error title and note. Available status options include pending review, needs attention, disputed, and released.

The Pay details card lets the admin edit function, hourly wage, hours worked, and travel expenses. The gross, loonheffing, deductions, and net amounts are shown as calculated summary values. The Deductions card is a line editor. Each line can have code, label, category, calculation type, configured value, manual override, sort order, notes, and calculated amount. Admins can remove lines or add a new deduction line.

The Employee details card keeps identity and address context visible. The Timesheets card shows timesheets for the same user, week, and year, with recorded hours and travel totals. This gives the admin a way to compare the payslip against the work-history data before saving changes.
...
The reviewer can download a PDF directly or click a row to open the admin payslip detail page. This screen is useful when the admin wants a narrow queue of review items instead of the broader all-payslips filtering interface.
...
The employee experience is deliberately self-service. It does not expose company-wide user lists, planning management, role configuration, or payroll editing. The app shows the employee what they need to know about their own schedule, hours, leave, expenses, and pay.

\subsection{Admin}

An admin has access to management pages. Admins see the Admin Dashboard by default, can search users in the navbar, browse the user directory, open employee detail pages, onboard employees, manage planning, review leave requests, inspect timesheets, review travel claims, and work with payslips.

Admins can also switch into personal view from the user menu. In personal view, the admin sees employee-facing pages for their own account. This is useful because admins may also be workers in the system, and it keeps their personal schedule and payslip experience separate from their admin work.

\subsection{Planning Manager}

Planning access is represented by \code{CAN_MANAGE_PLANNING}. A user with this permission can open Planning and Clients from the primary navigation. They can view the planning calendar, create events, manage clients, create shifts, edit shifts, assign employees, remove employees from shifts, and re-invite declined employees.
...
The user journey usually starts at the root URL or on the login page. The root URL is guarded and redirects active users to the dashboard, pending setup users to onboarding, and signed-out users to login. On the login page, the user enters credentials and submits the login form. The frontend then loads the current user and decides whether the user needs password reset, onboarding, or dashboard access. A forced password reset takes the user to the reset-password page, and that page can redirect back to the intended next step after the password is changed.

Once the user is signed in, access rules keep the flow clean. A pending setup user is redirected to onboarding. A signed-out user is redirected to login. A non-admin user cannot stay on admin pages. A user missing a required permission is sent back to dashboard. This creates a guided path through the app instead of exposing pages the user cannot use.

\subsection{Employee Invitation and Onboarding}

The admin creates an employee from the admin onboarding page. They enter the employee's personal, contact, function, wage, contract, and travel allowance information. When the form is submitted, the frontend displays the generated username, temporary password, and email result.
...
The workflow starts with an admin creating the account and ends with the employee becoming an active user who can manage their own planning, work history, leave, travel claims, and payslips.
...
After a shift is accepted and worked, the frontend represents the outcome through timesheet and planning states. Company workflow settings determine whether timesheets are expected to be finalized by an admin or logged automatically after the shift ends. The employee sees the result as Scheduled, Pending log, or Logged.
...
The result of this workflow is a worked-hours record that can inform dashboards, admin user details, travel claim review, and payroll.
...
After submission, the leave request appears in the employee's leave list as \code{PENDING}. Admins see pending leave requests on the Admin Dashboard. The admin opens the request, reviews dates, hours, available balance, remaining balance, and notes, then approves or rejects it. Approved leave reduces the employee's available leave hours shown on the dashboard.

\subsection{Payslip Review and Payslip Errors}

Payslips appear to employees in the dashboard and in the Payslips page. Employees can download PDFs and inspect details. If something looks wrong, the employee reports an error with a title and optional note. The payslip then becomes visible to admins as disputed or needing attention.

Admins can open a payslip from the dashboard, the payslip review page, or the all-payslips list. In the admin editor, they can review same-week timesheets, change status, edit error text, adjust pay details, edit deductions, and save. The employee-facing detail page then reflects the resulting payslip information.
...
Navigation is split between the top navbar and the primary navigation. The top navbar gives global account actions. It shows the ParadePaard brand, admin user search when allowed, a company menu when the user has company or role permissions, and the user avatar menu. The avatar menu shows the signed-in name, links to Account, supports logout, and lets admins switch between Admin Dashboard and Personal Account mode.

The admin user search in the navbar is a fast path to employee detail pages. An admin types part of a name, preferred name, or email, and the dropdown shows up to eight matches. Selecting a result opens that employee's detail page. The company menu opens Company Settings and is hidden in personal view, so company administration does not appear inside the admin's personal employee experience.

The primary navigation is the main page-to-page navigation inside the app shell. Everyone gets Dashboard, Work history, and Account. Employees and admins in personal view get My planning. Admins get Users and Onboarding. Planning managers get Planning and Clients. Payslip navigation appears only for users with payslip permissions. Travel Claims appears only for users with timesheet-management permission and is hidden in personal view.

Access checks make the navigation rules enforceable. The root route is guarded and forwards users to the right first page: dashboard for active sessions, onboarding for pending setup, or login for signed-out visitors. Active pages require an active signed-in user. Onboarding is reserved for pending setup users. Admin pages require admin status. Permission-based pages require the matching permission. When a user does not meet a page requirement, the frontend redirects them to the correct safe page, usually login, onboarding, or dashboard.
...
Modals support focused tasks without leaving the current page. They are used for leave requests, admin leave decisions, payslip error reporting, event and shift editing, client editing, role editing, and similar tasks. When a modal is open, the page behind it is prevented from scrolling, which keeps the user's attention on the current task.
...
The leave request modal is one of the most important shared workflow components. It turns dates, hours per day, weekend skipping, and available balance into a clear leave summary before submission. The admin request modal mirrors that logic for decision-making by showing requested hours, available hours, and remaining balance if approved.
...
The ParadePaard frontend tells one connected workforce story. An admin invites an employee and sets up the first contract details. The employee logs in, resets their password if needed, completes onboarding, and starts using the employee dashboard. A planner creates events and shifts, then assigns employees. Employees accept or decline those shifts. Completed shifts become worked-hour records, and those records appear in work history. Travel claims can be submitted after completed shifts and reviewed by managers. Worked hours, travel amounts, tax settings, and payroll review come together in payslips. Employees can download payslips or report errors, and admins can investigate and correct them.

The app is held together by role-aware navigation and page protection. Employees see the pages that help them manage their own work. Admins and authorized managers see the pages that help them manage people, planning, claims, settings, and payroll. The frontend therefore works as both an employee self-service portal and an admin operations console, with planning and payroll acting as the bridge between the two.
